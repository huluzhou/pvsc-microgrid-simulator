...
class MultiMeterClient:
    ...
    def __init__(self, base_port=403, meter_count=4, host="127.0.0.1"):
        ...
        for i, port in enumerate(self.meter_ports):
            try:
                ...
                if ok:
                    ...
                    print(f"   提示：请确认 Modbus TCP 服务器是否在 {host}:{port} 监听（当前报错多为端口未监听/被防火墙拦截）")
                    return False
...
# 仿真端在 Linux/WSL 无 root 时会把 403/404 映射到 10403/10404，客户端需与此一致
METER_BASE_PORT = 10403
...
def main():
    ...
    print("=" * 60)
    print("服务器: 127.0.0.1")
    print(f"端口: {METER_BASE_PORT}-{METER_BASE_PORT+3} (四个电表，与仿真端口映射一致)")
    print("寄存器: 地址0-1 (32位有功功率, 高低位组合)")
    print("-" * 60)
    print("📊 开始监控... 按 Ctrl+C 停止")
    print()
    multi_client = MultiMeterClient(base_port=METER_BASE_PORT, meter_count=2, host="127.0.0.1")
