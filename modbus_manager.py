...
class ModbusManager:
    ...
    def start_modbus_server(self, device_info):
        ...
        try:
            ...
            logger.info(f"✅ 已启动Modbus服务器: {device_info['name']} ({device_info['ip']}:{device_info['port']}) "
                       f"[类型: {device_info['type']}, 索引: {device_info['index']}]")
            return True
    ...
    def update_meter_context(self, index, slave_context):
        ...
        try:
            ...
            ae_export = int(float(getattr(meter_item, 'active_export_mwh', 0.0)) * POWER_UNIT / 50) & 0xFFFF
            ae_import = int(float(getattr(meter_item, 'active_import_mwh', 0.0)) * POWER_UNIT / 50) & 0xFFFF
            re_export = int(float(getattr(meter_item, 'reactive_export_mvarh', 0.0)) * POWER_UNIT / 50) & 0xFFFF
            re_import = int(float(getattr(meter_item, 'reactive_import_mvarh', 0.0)) * POWER_UNIT / 50) & 0xFFFF
    ...
    def start_all_modbus_servers(self):
        ...
        logger.info(f"准备启动Modbus服务器: 总计{len(self.ip_devices)}个设备 "
                   f"({', '.join([f'{k}:{v}' for k, v in device_counts.items()])})")
        
        for device_info in self.ip_devices:
            result = self.start_modbus_server(device_info)
            if result:
                logger.info(f"✅ 成功启动 {device_info['type']} 设备 {device_info['name']} "
                           f"({device_info.get('ip', 'N/A')}:{device_info.get('port', 'N/A')})")
            else:
                logger.warning(f"❌ 启动失败 {device_info['type']} 设备 {device_info['name']} "
                             f"({device_info.get('ip', 'N/A')}:{device_info.get('port', 'N/A')})")
        ...
        for device_info in self.ip_devices:
            try:
                ...
                if device_type == 'meter':
                    # 电表数据更新：检查measurement是否存在，以及索引是否匹配
                    if hasattr(self.network_model.net, 'measurement') and not self.network_model.net.measurement.empty:
                        # 尝试通过component_index匹配measurement索引
                        # 注意：电表的component_index可能与measurement.index不一致
                        # 需要通过element或其他属性来匹配
                        meter_found = False
                        if device_idx in self.network_model.net.measurement.index:
                            meter_found = True
                        else:
                            # 如果直接索引不匹配，尝试通过name或其他属性匹配
                            # 查找是否有measurement的name包含设备信息
                            for meter_idx in self.network_model.net.measurement.index:
                                meter_row = self.network_model.net.measurement.loc[meter_idx]
                                # 尝试通过name匹配（如果name包含设备索引）
                                if str(device_idx) in str(meter_row.get('name', '')):
                                    device_idx = meter_idx  # 使用匹配的measurement索引
                                    meter_found = True
                                    logger.debug(f"电表设备 {device_info['name']} 通过name匹配到measurement索引 {meter_idx}")
                                    break
                        ...
                        if meter_found:
                            try:
                                ...
                                logger.error(f"更新电表 {device_info['name']} (索引{device_idx}) 数据失败: {e}")
                        else:
                            logger.warning(f"电表设备 {device_info['name']} (component_index={device_idx}) 未在measurement中找到匹配项。"
                                         f"可用measurement索引: {list(self.network_model.net.measurement.index)}")
                    else:
                        logger.warning(f"网络模型中不存在measurement数据，无法更新电表 {device_info['name']} 的数据")
    ...
    def diagnose_meter_issues(self):
        """诊断电表Modbus通信问题
        ...
        diagnosis = {
            ...
            'meters_with_ip': 0,
            'meters_running': 0,
            'meters_in_measurement': 0,
            'issues': []
        }
        
        # 扫描电表设备
        self.scan_ip_devices()
        meter_devices = [d for d in self.ip_devices if d['type'] == 'meter']
        diagnosis['meters_found'] = len(meter_devices)
        diagnosis['meters_with_ip'] = len([d for d in meter_devices if d.get('ip') and d.get('ip') != '0.0.0.0'])
        diagnosis['meters_running'] = len([k for k in self.running_services if 'meter' in k])
        
        # 检查measurement索引匹配
        if hasattr(self.network_model.net, 'measurement') and not self.network_model.net.measurement.empty:
            measurement_indices = list(self.network_model.net.measurement.index)
            diagnosis['meters_in_measurement'] = len(measurement_indices)
            
            for device_info in meter_devices:
                device_idx = device_info['index']
                device_key = f"meter_{device_idx}"
                ...
                if device_key not in self.running_services:
                    diagnosis['issues'].append({
                        ...
                        'message': f"电表 {device_info['name']} 的Modbus服务器未运行"
                    })
                
                # 检查IP配置
                if not device_info.get('ip') or device_info.get('ip') == '0.0.0.0':
                    diagnosis['issues'].append({
                        'device': device_info['name'],
                        'type': 'no_ip',
                        'message': f"电表 {device_info['name']} 未配置IP地址"
                    })
                
                # 检查measurement索引匹配
                if device_idx not in measurement_indices:
                    # 尝试通过name匹配
                    matched = False
                    for meter_idx in measurement_indices:
                        meter_row = self.network_model.net.measurement.loc[meter_idx]
                        if str(device_idx) in str(meter_row.get('name', '')):
                            matched = True
                            break
                    ...
                    if not matched:
                        diagnosis['issues'].append({
                            'device': device_info['name'],
                            'type': 'index_mismatch',
                            'message': f"电表 {device_info['name']} (component_index={device_idx}) 未在measurement中找到匹配项。"
                                     f"可用measurement索引: {measurement_indices}"
                        })
        else:
            diagnosis['issues'].append({
                'device': 'ALL',
                'type': 'no_measurement',
                'message': "网络模型中不存在measurement数据"
            })
    ...
    def print_diagnosis_report(self):
        """打印诊断报告"""
        diagnosis = self.diagnose_meter_issues()
        
        logger.info("=" * 60)
        logger.info("电表Modbus通信诊断报告")
        logger.info("=" * 60)
        logger.info(f"发现电表设备: {diagnosis['meters_found']} 个")
        logger.info(f"配置IP的电表: {diagnosis['meters_with_ip']} 个")
        logger.info(f"运行中的电表服务器: {diagnosis['meters_running']} 个")
        logger.info(f"measurement中的电表: {diagnosis['meters_in_measurement']} 个")
        logger.info("")
