...
| 电表 | 母线、线路端口、变压器端口、功率设备连接点 | 每目标端口至多1个；每功率设备至多1个；不允许连接开关 |
...
│     └─ 功率设备已连接母线，再次连接母线 → 拒绝                   │
│     └─ 功率设备已连接电表，再次连接电表 → 拒绝                   │
│     └─ 线路/变压器端口已有母线或开关连接 → 拒绝                  │
│     └─ 电表已有连接 → 拒绝                                     │
├─────────────────────────────────────────────────────────────┤
│  4. 检查全局约束                                              │
│     └─ 外部电网设备数量已达上限（1个）→ 拒绝新建                 │
├─────────────────────────────────────────────────────────────┤
│  5. 检查稳态约束（警告，允许继续）                              │
│     └─ 开关两端均非母线 → 警告"稳态要求至少一端连接母线"          │
└─────────────────────────────────────────────────────────────┘
```
...
```json
{
  ...
  "from_port": "bottom",      // 源设备连接点
  "to_port": "center",        // 目标设备连接点
...
连接创建后，根据连接类型执行相应的联动操作：

```
┌─────────────────────────────────────────────────────────────────────────┐
│  连接类型               │  联动操作                                      │
├─────────────────────────────────────────────────────────────────────────┤
│  线路 ↔ 母线            │  • from_port="top" → 更新线路 from_bus          │
│                         │  • from_port="bottom" → 更新线路 to_bus         │
├─────────────────────────────────────────────────────────────────────────┤
│  变压器 ↔ 母线          │  • from_port="top" → 更新变压器 hv_bus          │
│                         │  • from_port="bottom" → 更新变压器 lv_bus       │
├─────────────────────────────────────────────────────────────────────────┤
│  开关 ↔ 母线            │  • 记录开关该端的母线索引                        │
│                         │  • 检查是否形成闭合连接并触发下游联动             │
├─────────────────────────────────────────────────────────────────────────┤
│  开关 ↔ 线路            │  • 记录开关连接元件类型为"线路"及线路索引          │
│                         │  • 若开关另一端已连母线，触发线路 bus 字段更新     │
├─────────────────────────────────────────────────────────────────────────┤
│  开关 ↔ 变压器          │  • 记录开关连接元件类型为"变压器"及变压器索引       │
│                         │  • 若开关另一端已连母线，触发变压器 bus 字段更新   │
├─────────────────────────────────────────────────────────────────────────┤
│  功率设备 ↔ 母线        │  • 更新功率设备的 bus 字段                        │
├─────────────────────────────────────────────────────────────────────────┤
│  电表 ↔ 任意设备        │  • 更新电表的 element_type、element、side 字段    │
└─────────────────────────────────────────────────────────────────────────┘
```

### 开关闭合连接的联动逻辑

当开关形成"母线—线路/变压器"的闭合连接时，需要同步更新：
...
| 删除连接类型 | 反向联动操作 |
|------------|-------------|
| 线路 ↔ 母线 | 清空线路对应端的 `from_bus` 或 `to_bus` |
| 变压器 ↔ 母线 | 清空变压器对应端的 `hv_bus` 或 `lv_bus` |
| 开关 ↔ 线路/变压器 | 清空开关的元件类型和索引；若影响闭合连接则清空对应 bus 字段 |
| 功率设备 ↔ 母线 | 清空功率设备的 `bus` 字段 |
| 电表 ↔ 任意设备 | 清空电表的 `element_type`、`element`、`side` 字段 |
